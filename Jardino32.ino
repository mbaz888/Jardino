...
#define mqtt_server_port 14083
#define mqtt_user "ejhqomgq"
#define mqtt_password "tlRjkknPdwqg"
...
void setup(void) {
 ...
 #define WIFI1_CHAR 1
  ...
  /*
  if (!wifiManager.autoConnect("Jardino32-Setup")) {
  ...
  for (int i = 0; i < n; ++i) {
    for (int j = 0; j < n_SSID ; j++){
      if (WiFi.SSID(i)== ssid[j] ) {
        WiFi.begin(ssid[j],password[j]); //trying to connect the modem
        wifi_ssid = ssid[j];
        Serial.println();
        Serial.print("Jardino connecting to: ");
...
void calculate_correction()
{
  ...
  if (configJardino.correction_ON and  (WiFi.status() == WL_CONNECTED)) 
  {
    ...
    if (wunderground.getCurrentTemp().length() > 0)
    {
      ...
      Serial.printf("WU Forecast: T %d -> %d , Ptoday %d mm , PoP %d %%\n",
                  current_temp,forecastHighTemp_1,precipitationToday,PoP_1);

      if      ( PoP_1 >= 80 and precipitationToday >= HIGH_RAIN_MM) // > 80% High Rain
    ...
    
    {
      ...
      if (rain >= HIGH_RAIN_MM || raining) // 5mm
      {
        //statusJardino.correction = -100;
        statusJardino.correction = -50;
        ground_humidity += 10;
      ...
      {
        statusJardino.correction = -30;
        ground_humidity += 1;
      }
      else if (data.temp > TEMP_HIGH or data.tempMax > TEMP_HIGH + 2)
      {
        statusJardino.correction  = +30;
        if (ground_humidity > 5) 
          ground_humidity -= 5;
        else
          ground_humidity = 0;
      }
      ...
      {
        statusJardino.correction  = 0;
        if (ground_humidity > 0) ground_humidity -= 1;
      }
    }

    if (ground_humidity > 20) statusJardino.correction = -100;
...
// ==========================================
// WIFI SETUP SCREEN ( No Refresh)
// ==========================================

void showWiFiSetupScreen(Input input)
{
  char buff[16];
  WiFiManager wifiManager;
  
  lcd.setCursor(1,0);
  lcd.print("WIFI SETUP Jardino32-Setup");
  lcd.setCursor(1,1);
  ...
  wifiManager.setConfigPortalTimeout(120);
  wifiManager.startConfigPortal("Jardino32-Setup");
...
void showInfoScreen(Input input)
{
  ...
  while (display_info_line < 4)
  {
    ...
    switch(info_line)
    {
      ...
      case 1:
        if (WiFi.status() == WL_CONNECTED){
          sprintf(buffer_str,"WIFI:%s",wifi_ssid);
          lcd.print(buffer_str); 
        }
        else
          lcd.print(" Not connected.");
